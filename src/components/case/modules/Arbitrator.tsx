...
export default function Arbitrator({ caseData }: ArbitratorProps) {
  ...
  const getArbitratorEmail = (arbitrator: any): string => {
    if (typeof arbitrator === 'object' && arbitrator !== null) {
      return arbitrator.email || '';
    }
    return '';
  };

  const arbitrators = caseData.arbitratorIds && Array.isArray(caseData.arbitratorIds) && caseData.arbitratorIds.length > 0
    ? caseData.arbitratorIds.filter(a => a !== null && a !== undefined)
    : [];
        ...
        {arbitrators.length > 0 ? (
          <div className="space-y-4">
            {arbitrators.map((arbitrator, index) => {
              const name = getArbitratorName(arbitrator);
              const email = getArbitratorEmail(arbitrator);
              return (
                <div key={index} className="p-6 border border-gray-200 rounded-lg bg-gray-50">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <p className="font-bold text-gray-900 text-xl mb-1">{name}</p>
                      ...
                      <span className="inline-block px-3 py-1 bg-orange-600 text-white text-sm rounded-lg font-semibold">
                        {arbitrators.length === 1 ? 'בורר' : index === 0 ? 'בורר ראשי' : `בורר ${index + 1}`}
                      </span>
                    </div>
                  </div>
                </div>
